...
pub fn parse_command(command: &str) -> Result<Command, &str> {
    ...
    let length = chunks.get(1).ok_or("Missing command length")?.trim_start_matches("$").parse::<i32>().map_err(|_| "Length must be an integer")?;
    ...
    match (*args, command_name.as_str()) {
        ...
        ("*2", "ECHO") => {
            let text_length = chunks.get(3).ok_or("Missing text length")?.trim_start_matches("$").parse::<i32>().map_err(|_| "Length must be an integer")?;
